...
\title{Title of the Paper Goes Here: Capitalize the First Letter of Major Words, Centered, 14-Point Palatino, on the Second Line from the Top Margin, Not More Than Three Lines Long}
...
\addAffiliation{1}{Name of Institute 1, Street, City, State}
\addAffiliation{2}{Name of Institute 2, Street, City, State}
\addAffiliation{3}{Name of Institute 2, Street, City, State}
\addAffiliation{4}{Name of Institute 4, Street, City, State}
...
\keywords{List of no more than five keywords}

%%% Provide a short title for the header on odd pages
\shortTitle{PHYSOR 2026 Full Paper Template (edit with your title)}

%%% Provide a short author listing for the header on even pages
\authorHead{A. A, et al. (edit with your initials)}
...
Use style ``Heading 1'' for section titles. Use 8.5" ×11" paper size, with 0.8" margins on all sides.
...
Recommended paper length is 6-8 pages.
\textbf{The limit for full-paper submissions is 8 pages, including references, tables, and figures.}
...
Do not include highlighting, page numbers, other headers or footers.
Disclaimers or information about the author’s employer should not be set as a footer or header.
Instead, set as an end-of-paper note (preferred) or as a footnote that does not interfere with the bottom margin. \textbf{Do not save your PDF as ``read only.''}
...
Use inline equations font with a font size 11 for the text variables and corresponding sizes proportionally for the subscripts and superscripts, for instance:
\begin{equation}\label{eq:BTE_3D}
    \frac{\partial f}{\partial t} + \vec v \cdot \nabla_{\vec r}f + \frac{\vec F}{m}\cdot \nabla_{\vec v}f = \dfrac{\partial f}{\partial t}_{|coll}.
\end{equation}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.3]{samplefigure.png}
    \caption{Portrait of the italian chemist Amedeo Avogadro, who spent his whole life in Torino.}
    \label{fig:sample}
\end{figure}

When importing figures or any graphical image please verify two things:
\begin{itemize}
    \item Any number, text or symbol is in Palatino font and is not smaller than 10 point after reduction to the actual window in the paper.
    \item That it can be translated into PDF.
\end{itemize}

Tables, such as Table I, are numbered in Roman numerals, with the table title in \textit{italics} centered above the table with a blank line between the title and actual table. The style for tables is ``Plain Table 2'' found under the ``Table Design'' tab. The style for table captions is ``Table Caption''. The text style for table content is ``Table''.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{lccc}
        \toprule
         \textbf{Mesh}           & \textbf{8 $\times$ 8} & \textbf{16 $\times$ 16} & \textbf{32 $\times$ 32} \\ \midrule
         \textbf{Nodal}          & $1.000\times 10^{-1}$ & $2.500\times 10^{-2}$ & $6.250\times 10^{-3}$ \\ \midrule
         \textbf{Characteristic} & $1.000\times 10^{-1}$ & $2.500\times 10^{-2}$ & $6.250\times 10^{-3}$ \\ \bottomrule
    \end{tabular}
...
The organizers of PHYSOR 2026 wish to thank the organizers of PHYSOR 2024 for having shared their original template.
