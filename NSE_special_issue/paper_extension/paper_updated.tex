...
\begin{abstract}
The fission matrix (FM) method enables whole-core reactor transport calculations within seconds by interpolating a precalculated database of Monte Carlo (MC)--tallied coefficients across different operational parameters instead of running a direct MC calculation for every configuration of interest. In this work, we focus on the parametrization of the FM with respect to the temperature distribution in the core. Interpolation implicitly assumes that each coefficient depends only on the local temperature of cells where fission occurs. This assumption breaks down under large, spatially sharp temperature gradients. In this work, a Core Temperature Correction Ratio (CTCR) is introduced to compensate this effect. A point-estimate improvement alone, however, cannot distinguish a real, uncorrected bias from the statistical noise inherent to the underlying MC calculations. This requires an uncertainty budget with two components: the reference case's MC statistics, and the uncertainty of the \emph{interpolated} matrix itself: a new object, assembled from the database's own finite-statistics matrices, which carries no MC replicas of its own. We construct the latter by resampling the database and recomputing the interpolated (and corrected) result many times, comparing an uncorrelated baseline, an innovative derivation of a zero-replica analytical model, a measured-correlation model, and a genuine brute-force reconstruction from 150 independent replicas. Validated against brute force across 12 test cases, the analytical model reproduces the true $k_{\text{eff}}$ uncertainty to within a mean absolute deviation of 4.6\%, matching or exceeding the two more expensive alternatives, while requiring no MC replicas beyond the nominal database already needed for interpolation. Combining both uncertainty sources, the benefits of the CTCR are evident in almost all cases. In general, it removes most of the point-estimate $k_{\text{eff}}$ discrepancy for sharply varying, highly localized temperature fields, whereas the improvement becomes less pronounced (and less necessary) for smoother temperature distributions. The local fission-source error follows the same pattern, improving markedly wherever a strong, spatially localized temperature gradient is present; where the correction succeeds, the residual discrepancy is generally consistent with the combined statistical noise of the reference case and the interpolated database.
\end{abstract}

\section{Introduction}
\label{sec:intro}
Monte Carlo (MC) methods provide the highest-fidelity treatment of the linear Boltzmann transport equation for neutrons in reactor physics applications, at the cost of a statistical uncertainty that decreases only as the inverse square root of the simulated particle histories. Hybrid neutron transport methods based on the fission matrix (FM) formulation~\cite{CarneyANE2014} address this cost by precalculating a database of FM coefficients under a set of reference system configurations and then interpolating and combining these coefficients to represent an arbitrary configuration within the database validity range. 

The RAPID code~\cite{WaltersNSE2018} is a mature implementation of this approach, yielding whole-core transport solutions within a few minutes at accuracy comparable to the underlying MC calculations, aided by dedicated techniques for FM homogenization~\cite{HePNE2020Homog}, temperature interpolation~\cite{HeANE2020Temp}, and correction of material and control-rod discontinuity effects~\cite{HeANE2019,HePNE2020}.

RAPID assembles its whole-core fission matrix from fixed-source calculations performed in a single fundamental assembly unit, exploiting the geometrical and material similarities between repeated units to limit the required database calculations; reflectors and structural materials are deliberately excluded from the FM tally to preserve this similarity, and their influence is recovered afterward through dedicated boundary and discontinuity corrections. The present work adopts a different construction: the full-core fission matrix is tallied directly from a single whole-core criticality calculation in Serpent~\cite{LeppanenEPJN2025}, so that reflectors, structural materials, and inter-assembly heterogeneity effects are already embedded in each coefficient, and a complete fission matrix is obtained from one calculation rather than assembled from fundamental-unit responses.

The system studied here is a modified Heat-Pipe Micro-Reactor (HPMR) concept, and the operational variable of interest is the cell temperature: the FM database is tallied at a set of uniform reference temperatures, and an arbitrary in-core temperature field is represented by interpolating each coefficient across that database. As a general approach widely adopted to deal with interpolations performed on temperature distributions, it is assumed that a coefficient $a_{ij}$ depends only on the \emph{local} temperature of the departure cell, i.e. the $i$-th cell. However, under a large, spatially sharp temperature gradient that assumption breaks down and the interpolated matrix acquires a systematic error. To compensate it, we introduce a Core Temperature Correction Ratio (CTCR), calibrated once from two additional FMs, and show that it reduces the point-estimate discrepancy between interpolated and directly-simulated reference results across a range of test cases.

To better understand the systematic error and the improvement provided by the CTCR, a point-estimate comparison of the interpolated FMs and the reference one directly calculated with Serpent could not be enough to distinguish a real, uncorrected modeling bias, since both the database and the reference case are generated by Monte Carlo simulations, which always exhibit intrinsic statistical noise. Making that distinction requires quantifying the statical uncertainty on both quantities being compared. This estimation is straightforward for the reference case: Serpent~\cite{LeppanenEPJN2025} naturally reports its own statistical uncertainty for each tallied quantity, including both $k_{\text{eff}}$ and the fission source, $F$. On the contrary, the interpolated matrix is not provided with any uncertainty: the matrix produced by interpolation (and CTCR correction) is a new object, assembled from the database's finite-statistics matrices, and unlike a direct MC run it has no replicas of its own from which an uncertainty could be read off. The second aim of this work is centered on this practical gap, i.e. constructing a computationally smart uncertainty estimate for the interpolated FM. This will allow us to better quantify any systematic error and the improvement provided by the correction ratio.  
In the next sections a detailed description of the database construction and the interpolation pipeline are described with the CTCR definition; the combined uncertainty source (reference and interpolated cases) problem is described; the four approaches considered for estimating the database-side contribution, and the error metrics adopted throughout. Finally, the reference test cases are described and the corresponding error metrics are reported.
...
The fission matrix (FM) method~\cite{CarneyANE2014} recasts the $k$-eigenvalue transport problem into a compact linear-algebraic form by partitioning the fissile domain into $N$ spatial regions and describing the transport process among them over a single but statistically representative neutron generation. Applying the integral form of the $k$-eigenvalue transport equation over the volumes of a source region $j$ and a destination region $i$ yields an algebraic eigenvalue problem,
\begin{equation}
...
Eq.~\ref{eqn:interp} treats each coefficient as depending only on the local temperatures $i$, and therefore misses the influence of the surrounding cells. Under a large, spatially sharp gradient this is precisely where the interpolated matrix acquires a systematic error. The Core Temperature Correction Ratio (CTCR) defined in this work tries to compensate this by informing the numerical value of a generic matrix element $a_{ij}$, based on the couple of  temperatures $(T_i,T_j)$ through a correction ratio $\hat r_{ij}$.
...
Because the correction must handle both a locally \emph{hot} cell embedded in a colder surrounding and the reverse case, both Dirac FMs of Figure~\ref{fig:dirac_distributions} are used: one with the central assembly $C$ driven to the hottest database temperature (1000~K) while the rest of the core sits at the coldest temperature (300~K), and one with the assignment reversed. Comparing each Dirac FM against its own locally-interpolated counterpart gives two ratios, each a function of the geometric distance $d$ between $C$ and a generic cell $j$, including $j=C$ itself ($d=0$) (exploiting the hexagonal core's sextant symmetry, so that only $d$, not direction, matters),
\begin{equation}
  ...
  \label{eqn:ratio_def}
\end{equation}
where the superscript $+$ ($-$) denotes the hot-center (cold-center) Dirac case, and $a^{\text{interp}}_{Cj}$ is the coefficient the ordinary interpolation of Eq.~(\ref{eqn:interp}) would give for that same, uniform-elsewhere temperature field. Each ratio in isolation quantifies how much the local-temperature interpolation assumption misses, in the most extreme, the most spatially sharp case the database can represent; combined, the two make the correction able to handle a temperature difference of either sign.
...
\begin{equation}
  ...
  \label{eqn:ctcr}
\end{equation}
where $\Delta T_{\text{dirac}} = 700$~K is the calibration temperature span common to both Dirac cases, and $r^{+}(d)$ is used when $\Delta T_{kl}>0$ (cell $k$ hotter than $l$, or than its own first-ring neighborhood average $\mathcal{N}(k)$ when $k=l$) while $r^{-}(d)$ is used when $\Delta T_{kl}<0$ --- i.e., the calibration case matching the \emph{sign} of the local temperature difference, evaluated at its own geometric distance $d$. For a diagonal coefficient ($k=l$, $d=0$) this reuses $r^{\pm}(0)$ from the same Dirac calibration of Eq.~\ref{eqn:ratio_def}, so no additional simulation or calibration data is needed to cover this case.
...
In this work four different approaches are introduced to propagate the database uncertainty to the interpolated FM results. Three of these methods share the same recipe: the nominal eight-temperature database is perturbed with random noise, the interpolator and correction of Eq.~(\ref{eqn:ctcr}) are reapplied, and the procedure is repeated many times. The spread of the resulting $k_{\text{eff}}^{\text{interp}}$ and fission source $F^{\text{interp}}$ over these repetitions is the database-side uncertainty. % $\sigma_{\text{database}}$.

The four approaches differ in how the random perturbation is enforced. Indicating the perturbation of the retained coefficients as a vector $\boldsymbol\delta$ drawn from a zero-mean multivariate normal distribution, $\boldsymbol\delta\sim\mathcal{N}(\mathbf{0},\boldsymbol\Sigma)$, the covariance $\boldsymbol\Sigma$ factors as
\begin{equation}
...
\begin{enumerate}
      \item[(i)] \textbf{Uncorrelated resampling ($\boldsymbol\rho=\mathbf{I}$).} Each retained coefficient is perturbed independently using only its own MC-reported variance, $a_{ij}\to a_{ij}+\delta_{ij}$ with $\delta_{ij}\sim\mathcal{N}(0,\sigma_{ij}^2)$. This is equivalent to setting $\boldsymbol\rho$ to the identity and is the simplified assumption adopted in prior FM uncertainty-quantification work~\cite{HeWaltersNSE2022}. It serves as the natural baseline against which to evaluate the impact of modeling correlation.

    \item[(ii)] \textbf{Analytical model ($\boldsymbol\rho=\boldsymbol\rho_{\text{pred}}$).} The correlation matrix is predicted in closed form, $\boldsymbol\rho_{\text{pred}}$, derived directly from the branching structure of the fission process. To the best of our knowledge, this closed-form prediction of the fission-matrix correlation structure from the branching process, and its use to build the interpolated matrix uncertainty, has not been reported before: prior fission-matrix uncertainty-quantification works either neglect inter-coefficient correlation or estimate it empirically. The approach requires no Monte Carlo replicas beyond the single nominal database already needed for interpolation.

    \item[(iii)] \textbf{Correlated resampling, measured correlation ($\boldsymbol\rho=\hat{\boldsymbol\rho}$).} The correlation matrix is instead measured from the cycle-by-cycle tallies of a single MC run, treating each active cycle as a pseudo-replica of the same underlying random vector. Since Serpent does not provide the fission matrix on a per-cycle basis, this required a modification of the source code to tally and print each active cycle's fission matrix separately.

  This $\hat{\boldsymbol\rho}$ is measured once, at 300~K, and reused unchanged at every other database temperature, with an analysis of the reasonableness of this approach.

  \item[(iv)] \textbf{Brute force.} The full eight-temperature database is rebuilt from scratch $N=150$ times, with one genuinely independent MC replica per temperature per repetition, reapplying the interpolator and correction each time. This bypasses the $\boldsymbol\Sigma=\mathbf{D}\boldsymbol\rho\mathbf{D}$ construction entirely: all correlations are present in the replicas by construction, with no modeling assumption. It is the most direct and most expensive estimate, and serves as the reference against which approaches (i)--(iii) are validated.
\end{enumerate}
...
The 300~K vs. 1000~K distance is \emph{smaller} than the noise floor of estimating $\boldsymbol\rho$ from half the data at a single, fixed temperature. By this measure we can state that the correlation matrix changes less between the two temperature extremes than a single-temperature estimate fluctuates under its own finite statistics, so reusing one matrix across the whole database is at least as reasonable as trusting the estimate at any one temperature on its own.


\subsection{Definition of error metrics}
\label{sec:metrics}

Two plain metrics are used throughout, deliberately without a covariance-weighted quadratic form such as a Mahalanobis distance: they are less sensitive to the \emph{spatial pattern} of a residual, but directly interpretable, which is what matters for our case-by-case comparison. For $k_{\text{eff}}$,
\begin{equation}
...
\end{equation}
The fission source $F$ is normalized to unit sum over the 127 cells giving $\tilde{F_i}$. Its local relative deviation in each zone is defined as
\begin{equation}
    \qquad  \delta \tilde{F_i} = \frac{\tilde{F_i}^{\text{interp}} - \tilde{F_i}^{\text{ref}}}{\tilde{F_i}^{\text{ref}}}\times 100\%,
...
\begin{equation}
  \mathrm{RMS} = \sqrt{\left\langle \delta \tilde{F_i}^2 \right\rangle_i}.
...
The pipeline is applied on twelve validation cases, each a different in-core temperature field imposed on the HPMR model, reported in Figure~\ref{fig:test_case_temperature_distribution_1} and Figure~\ref{fig:test_case_temperature_distribution_2}. They are chosen to span a wide range of spatial sharpness, since it is precisely under sharp gradients that the local-temperature interpolation assumption -- and hence the CTCR -- is most severely tested. The set comprises two spatially sharp ``Dirac'' perturbations, in which a single central assembly is offset in temperature from an otherwise uniform core; a six-assembly and two ring-shaped hot-spot patterns; a smooth linear gradient across the core; and six power-shaped profiles of varying peak sharpness and radial offset. Each case is evaluated in both the \emph{raw} (interpolation only) and \emph{corrected} (interpolation plus CTCR) variants, against a direct Serpent reference calculation of the same temperature field.
...
\begin{figure}[h!]
  ...
  \caption{Database-side uncertainty predicted by the different $\sigma_{\text{interp}}$ quantification methods against the brute-force reference (iv, $N=150$): $k_{\text{eff}}$ (left) and fission-source RMS (right), corrected variant, all twelve cases.}
  \label{fig:fig2}
\end{figure}


On $k_{\text{eff}}$, the analytical model is the closest of the three, reproducing the brute-force uncertainty to within 4.6\%, ahead of the single-seed measured model (8.7\%) and well ahead of the uncorrelated model (18.0\%). It can be noticed that the intra-column correlations are negative, so neglecting them (method i) inflates the propagated $k_{\text{eff}}$ variance, while capturing them in closed form (method ii) does not require the additional simulation that measuring them (method iii) does. On the fission-source RMS the ranking changes: the single-seed (3.3\%) and the analytic (3.6\%) models track brute force more closely than the uncorrelated one (7.3\%). This is consistent with the cross-column residual left unmodeled by method (ii): the fission source is a less global quantity, sensitive to correlations between coefficients in different columns in a way the eigenvalue is not, so the very terms the analytical model omits matter more here.

Despite not being uniformly best, the analytical model is adopted for both observables in the combined uncertainty value: on both it remains within the same order of magnitude as the brute-force reference, and it alone requires no Monte Carlo replicas beyond the nominal database: the practical criterion this work is built around.
...
% The seed-to-seed spread is negligible: two to three orders of magnitude below both the temperature dependence of the measured matrix (0.643) and its single-temperature noise floor (0.909, Sec.~\ref{sec:t_invariance}).
The Monte Carlo noise in the mean FM coefficients is therefore not a meaningful source of uncertainty for the analytical model, and $\boldsymbol\rho_{\text{pred}}$ can be treated as effectively deterministic.


\subsection{Does the Correction Ratio Help at the Point-Estimate Level?}
\label{sec:res_raw_vs_corrected}

Figure~\ref{fig:fig3} compares raw and corrected interpolation against the direct reference, before any uncertainty is folded in, and Table~\ref{tab:reference_results} reports the underlying per-case numbers. Averaged over the twelve cases, the CTCR reduces the mean absolute $k_{\text{eff}}$ discrepancy from 40.8 to 23.7~pcm (42\%) and the mean fission-source RMS discrepancy from 1.27\% to 0.94\% (26\%). The benefit is markedly uneven. It is largest for the sharpest, most localized profiles --- Rings 1000~K improves from $57.4$ to $1.2$~pcm, and 6~assemblies 1000~K from $53.9$ to $24.0$~pcm --- consistent with the CTCR having been calibrated against a sharp central perturbation. For smoother, less localized profiles it is negligible or mildly unfavorable, the Power Tmin650 DT200 case worsening from $-41.5$ to $-52.8$~pcm. This geometric dependence is expected from the way the correction is constructed: calibrated on a Dirac-like perturbation, it is most effective where the true field most resembles one.
...
Dirac 1000~K            & 79.7  & 13.5  & 0.972 & 0.838 \\
Dirac 300~K              & $-$55.4 & $-$15.4 & 1.064 & 0.975 \\
6 assemblies 1000~K      & 53.9  & 24.0  & 1.451 & 0.794 \\
Rings 1000~K             & 57.4  & 1.2   & 2.411 & 1.052 \\
Rings 600/1000~K         & 42.0  & 19.5  & 2.084 & 1.244 \\
Linear 300--900~K        & $-$46.3 & $-$45.8 & 1.525 & 1.212 \\
Power shape 300--1000~K  & 31.7  & $-$7.4  & 1.828 & 1.456 \\
Power Tmin650 DT100      & 11.7  & 6.0   & 0.680 & 0.688 \\
Power Tmin650 DT200      & $-$41.5 & $-$52.8 & 0.772 & 0.748 \\
Power Tmin650 DT300      & $-$5.2  & $-$22.2 & 0.932 & 0.806 \\
Power spike Tmin350      & $-$18.4 & $-$24.3 & 0.909 & 0.775 \\
Power inverse Tmin650    & 46.0  & 51.8  & 0.671 & 0.670 \\ \hline
Mean $|\cdot|$           & 40.8  & 23.7  & 1.275 & 0.938 \\ \hline
\end{tabular}
\end{table}
...
Combining $\sigma_{\text{ref}}$ with $\sigma_{\text{interp}}$ from the analytical model in quadrature gives the total uncertainty $\sigma_{\text{tot}}$ of Eq.~(\ref{eqn:sigma_tot}). Under this combined budget, 58\% of the corrected cases fall within $|z|<2$ (median $|z|=1.30$), against 42\% of the raw cases (median $|z|=2.18$): a consistent and, for the corrected variant, now a majority of cases. $\sigma_{\text{interp}}$ is 14--22~pcm against 9--12~pcm for $\sigma_{\text{ref}}$ in every case, so the database term still dominates the budget and both variants are judged against a relatively wide band.

Rather than collapsing this budget to a single $\pm2\sigma_{\text{tot}}$ number per case, Figure~\ref{fig:fig4} shows the full picture for $k_{\text{eff}}$: every one of the $N=500$ analytical-model realizations (gray) and, since 150 genuinely independent brute-force replicas of the whole database already exist, every one of those true replicas too (blue open circles), alongside the reference case's own $\pm2\sigma_{\text{ref}}$ (pink band) and the raw/corrected point estimates (black/red). 
% Collapsing this to a single reported standard deviation, as in a conventional error-bar plot, would hide \emph{where} a case's point estimate actually falls within its own uncertainty: a case can sit near the edge of its distribution, or comfortably at its center, and only the full cloud shows which.
\begin{figure}[h]
  \centering
  \includegraphics[width=150mm]{NSE_special_issue/paper_extension/figures/fig4_keff_points_distribution_vs_truth_combined_mod_chri.png}
...
For the fission source (Fig.~\ref{fig:fig5_frist_6_cases}) the same full-distribution logic is applied but this time, only the brute force method cell-wise result is shown, because the $N=500$ synthetic analytical realizations, of every $F_i$, add visual clutter without adding information beyond what is already displayed in ~\ref{fig:fig2}, so only the $N=150$ genuine, independent brute-force replicas are shown, split by flavor: raw in gray, corrected in shades of red. A corrected point is recolored gold if and only if it falls inside that band \emph{and} the matching raw point, of the same zone, was outside it: exactly the population for which the CTCR turned a discrepancy raw could not explain by noise into one it can. Every other corrected point --- already consistent with noise in raw, or still not explained by noise even after correction --- keeps the base red. This isolates the population the correction is meant to act on, rather than diluting the plot with every in-band point regardless of whether raw needed fixing there at all. Figure~\ref{fig:hex_map} reports the core assemblies numbering to better navigate the results in Fig.~\ref{fig:fig5_frist_6_cases} where only the first six test cases are displayed for sake of shortness, but still well displays both cases, where the CTCR has an higher impact on local quantity $F_i$, and where temperature differences are less sharp and so the CRCT less needed. The remaining six cases are reported in \ref{fig:fig5_appendix_b}).
...
\begin{figure}[h]
  \centering
  \includegraphics[width=80mm]{figures/hex_index_map.png}
  \caption{Hexagon-index map (1--127) referenced by the zone-index axis of Fig.~\ref{fig:fig5_frist_6_cases} and Fig.~\ref{fig:fig5_appendix_b}; hexagon 64 is the central assembly $C$ of the Dirac calibration cases.}
  \label{fig:hex_map}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=150mm]{NSE_special_issue/paper_extension/figures/fig5a_fission_source_brute_force_rescue_mod_chri.png}
  \caption{$\delta F_i$ of the raw (red dots) and corrected (black dots) cases. The softer colored dots represent the dispersion of $\delta F_i$ using the brute-force method only. Red points are turned gold only when are inside the $\pm2\sigma_{\text{ref}}$ band and any of the black dots of the same region are.}
  \label{fig:fig5_frist_6_cases}
\end{figure}

This systematic offset of the whole replica distribution away from zero is itself informative: it points to a mismatch between the database and some of the test cases that no amount of resampling the database can correct, because it is not database noise at all. The database is tallied with the radial reflector at the same uniform temperature as the rest of the core at each of its eight reference points, whereas several test cases instead fix the reflector at a constant 300~K regardless of the in-core field being tested. Where the in-core temperature departs from 300~K, this creates a sharp, systematic core-to-reflector gradient that is simply absent from the database's own construction, and every replica of that database --- brute-force or synthetic --- inherits the same mismatch, hence the same offset. This is a bias in the comparison's reference frame, not a statistical fluctuation.

Where the corrected replica cloud is centered close to zero --- Dirac 300~K, Linear 300--900~K, and most of the smoother power-shaped profiles --- the residual deviation from the reference is statistically indistinguishable from Monte Carlo noise (and the reflector-mismatch bias just described, where present, is small relative to that noise), and no further correction is warranted. Where it is not --- the localized spikes in Dirac 1000~K, 6-assemblies 1000~K, and the Rings cases --- the deviation exceeds what either noise source can account for, and is therefore a genuine, uncorrected bias rather than statistical scatter. Because the database-noise model has itself been validated against brute force, it is not the plausible origin of these residuals; the most likely cause is the interpolation scheme's finite temperature grid, whose eight-point spacing cannot fully resolve the sharpest gradients. Figure~\ref{fig:hex_map} maps each zone index in Fig.~\ref{fig:fig5_frist_6_cases} and back to its physical hexagon.
...
The pattern is consisten with the point-estimate result of Table~\ref{tab:reference_results}, but is sharper here: for the Rings cases the corrected count roughly doubles (e.g., Rings 1000~K rises from 36--41 to 77--88 zones out of 127, consistently across all four models), while for the smoothest profiles the shift is small or even mildly unfavorable (Dirac 300~K, Power Tmin650 DT100/DT200). It is important to emphasize the accurate reproduction of local quantities, since in many transient analyses the quantity of interest is not a core-averaged parameter but rather the most critical point in the system, such as a localized hot spot. In these situations, the reliability of the interpolated model is determined by its ability to accurately predict the behavior at that specific location, rather than by the overall agreement in terms of global quantities such as $k_{\text{eff}}$ or the RMS error. Indeed, it is the accuracy at the most critical zone—where an error would have the greatest impact—that ultimately determines whether the model can be trusted.
...
%   \centering
%   \includegraphics[width=80mm]{figures/hex_index_map.png}
%   \caption{Hexagon-index map (1--127) referenced by the zone-index axis of Fig.~\ref{fig:fig5} and Figs.~\ref{fig:fig5_appendix_a}--\ref{fig:fig5_appendix_b}; hexagon 64 is the central assembly $C$ of the Dirac calibration cases.}
%   \label{fig:hex_map}
% \end{figure}

% \begin{figure}[h]
%   \centering
%   \includegraphics[width=150mm]{figures/fig5_fission_source_selected.png}
%   \caption{Relative error of $F_i$ of the raw (red dots) and corrected (black dotes) cases. The results dispersion is computed relying on true brute-force method only. The corrected points is colored gold when is within the $\pm2\sigma_{\text{ref}}$ and all the raw points of the same cell were outside it. So, if some black dots are already inside the reference case uncertainty band, none of the red dots will be switched gold. Six representative cases, each row pairing a large rescued fraction (left) against a nearly unaffected case (right): Rings 1000~K / Power Tmin650 DT100, Rings 600/1000~K / Power Tmin650 DT200, 6~assemblies 1000~K / Power inverse Tmin650.}
%   \label{fig:fig5}
% \end{figure}
...
\section*{Appendix: Fission-Source Deviation, All Twelve Cases}

Fig.~\ref{fig:fig5_frist_6_cases} in the main text shows the first six test cases, used to verify the CTCR effect. For completeness, Fig.\ref{fig:fig5_appendix_b} give the same plot, in the same style, for the remaining six test cases.
...
%   \label{fig:fig5_appendix_a}
% \end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=150mm]{NSE_special_issue/paper_extension/figures/fig5b_fission_source_brute_force_rescue_mod_chri.png}
  \caption{As Fig.~\ref{fig:fig5_frist_6_cases}, for test cases 7--12.}
  \label{fig:fig5_appendix_b}
\end{figure}
