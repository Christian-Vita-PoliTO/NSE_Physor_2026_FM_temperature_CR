...
A global summary metric can be misleading about how well the correction is actually doing its job locally. Sec.~\ref{sec:res_raw_vs_corrected} already reported the CTCR's benefit on the two aggregate figures of merit, $k_{\text{eff}}$ and the fission-source RMS: both improve, but a global metric averages over all 127 zones, and so cannot distinguish a case that improves a little everywhere from one that improves dramatically in exactly the zone that matters and not at all elsewhere. Table~\ref{tab:hexcount} makes the zone-by-zone picture concrete: pooling all twelve cases (1524 zone instances), the fraction of zones whose fission-source point estimate is statistically consistent with noise ($|\delta F_i|<2\sigma_{\text{tot}}$) rises from 70--73\% (raw) to 78--82\% (corrected), depending on which of the four $\sigma_{\text{database}}$ models of Sec.~\ref{sec:four_methods} is used to build the band --- a shift observed consistently across all four, so it is not an artifact of which noise model happens to be used to judge it. The ensemble columns of Table~\ref{tab:hexcount} report the same statistic computed per replica and averaged over the $N$ replicas of each model, rather than for the single nominal point alone, and show the same consistent improvement. This distinction matters beyond bookkeeping: for a transient analysis, where the quantity of interest is often a single, localized hot spot --- the most critical point of the system --- rather than any core-averaged figure, it is representativeness at that one zone, not how good the global $k_{\text{eff}}$ or the RMS looks on average, that determines whether the interpolated model can be trusted where an error would matter most.

\begin{table}[h]
\centering
\caption{Fraction of Zones (1524 = 127 zones $\times$ 12 cases) Statistically Consistent with Noise ($|\delta F_i|<2\sigma_{\text{tot}}$), by $\sigma_{\text{database}}$ Model}
\label{tab:hexcount}
\begin{tabular}{lcccc}
\hline
 & \multicolumn{2}{c}{Point estimate} & \multicolumn{2}{c}{Ensemble (per-replica average)} \\
$\sigma_{\text{database}}$ model & raw & corrected & raw & corrected \\ \hline
(i) Uncorrelated   & 73.0\% & 81.6\% & 65.0\% & 70.7\% \\
(ii) Analytical    & 69.7\% & 77.5\% & 64.5\% & 72.1\% \\
(iii) Measured     & 70.9\% & 78.7\% & 63.5\% & 69.2\% \\
(iv) Brute force   & 72.0\% & 80.6\% & 71.3\% & 80.3\% \\ \hline
\end{tabular}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=160mm]{figures/fig5a_fission_source_brute_force_rescue.png}
