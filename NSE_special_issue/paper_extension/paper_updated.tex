...
\begin{equation}
  ...
  \label{eqn:ctcr}
\end{equation}
where $\Delta T_{\text{dirac}} = 700$~K is the calibration temperature span common to both Dirac cases, and $r^{+}(d)$ is used when $\Delta T_{kl}>0$ (cell $k$ hotter than $l$) while $r^{-}(d)$ is used when $\Delta T_{kl}<0$ --- i.e., the calibration case matching the \emph{sign} of the local temperature difference, evaluated at its own geometric distance $d$. The corrected FM is renormalized column-wise so that each cell's total fission source contribution is preserved, following the same convention as Ref.~\cite{VitaPHYSOR2026}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=145mm]{figures/fig5a_temperature_distributions.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=145mm]{figures/fig5b_temperature_distributions.png}
  \caption{Reference-cases temperature distribution.}
...
Dirac 1000~K            & 79.7  & 8.5  & 0.972 & 0.844 \\
Dirac 300~K              & $-$55.4 & $-$21.4 & 1.064 & 0.979 \\
...
Rather than collapsing this budget to a single $\pm2\sigma_{\text{tot}}$ number per case, Figures~\ref{fig:fig4} and~\ref{fig:fig5}--\ref{fig:fig5b} show the full picture: every one of the $N=500$ analytical-model realizations (gray) and, since 150 genuinely independent brute-force replicas of the whole database already exist (Sec.~\ref{sec:four_methods}), every one of those true replicas too (blue open circles), alongside the reference case's own $\pm2\sigma_{\text{ref}}$ (pink band) and the raw/corrected point estimates (black/red). Collapsing this to a single reported standard deviation, as in a conventional error-bar plot, would hide two things the full distribution makes visible directly. First, \emph{where} a case's point estimate actually falls within its own uncertainty: a case can sit near the edge of its distribution, or comfortably at its center, and only the full cloud shows which. Second, for the fission source specifically (Fig.~\ref{fig:fig5}--\ref{fig:fig5b}), that even the brute-force replica distribution itself --- not just the raw or corrected point estimate --- is systematically offset from zero relative deviation in several cases, rather than straddling it symmetrically; a $\pm2\sigma$ band centered on the point estimate would not reveal this, since it says nothing about where the underlying ensemble itself is centered.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=150mm]{figures/fig5a_fission_source_ensemble_vs_truth.png}
  \caption{Fission-source relative deviation from the reference, zone by zone: every analytical-model realization (gray, $N=500$) and every true brute-force replica (blue, $N=150$), raw (black) and corrected (red) point estimates, with the reference case's own $\pm2\sigma_{\text{ref}}$ (pink band); test cases 1--6.}
  \label{fig:fig5}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=150mm]{figures/fig5b_fission_source_ensemble_vs_truth.png}
