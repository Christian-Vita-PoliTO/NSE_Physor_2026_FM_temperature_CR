...
Where the corrected replica cloud is centered close to zero --- Dirac 300~K, Linear 300--900~K, and most of the smoother power-shaped profiles (Appendix, Figs.~\ref{fig:fig5_appendix_a}--\ref{fig:fig5_appendix_b}) --- the residual deviation from the reference is statistically indistinguishable from Monte Carlo noise (and the reflector-mismatch bias just described, where present, is small relative to that noise), and no further correction is warranted. Where it is not --- the localized spikes in Dirac 1000~K, 6-assemblies 1000~K, and the Rings cases --- the deviation exceeds what either noise source can account for, and is therefore a genuine, uncorrected bias rather than statistical scatter. Because the database-noise model has itself been validated against brute force (Sec.~\ref{sec:validation}), it is not the plausible origin of these residuals; the most likely cause is the interpolation scheme's finite temperature grid, whose eight-point spacing cannot fully resolve the sharpest gradients. Figure~\ref{fig:hex_map} maps each zone index in Fig.~\ref{fig:fig5} and Figs.~\ref{fig:fig5_appendix_a}--\ref{fig:fig5_appendix_b} back to its physical hexagon.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=90mm]{figures/hex_index_map.png}
  \caption{Hexagon-index map (1--127) referenced by the zone-index axis of Fig.~\ref{fig:fig5} and Figs.~\ref{fig:fig5_appendix_a}--\ref{fig:fig5_appendix_b}; hexagon 64 is the central assembly $C$ of the Dirac calibration cases (Sec.~\ref{sec:pipeline}).}
  \label{fig:hex_map}
\end{figure}
...
Fig.~\ref{fig:fig5} in the main text shows six representative cases, paired for contrast. For completeness, Figs.~\ref{fig:fig5_appendix_a}--\ref{fig:fig5_appendix_b} give the same plot, in the same style, for all twelve.
...
\begin{figure}[h]
  ...
  \caption{As Fig.~\ref{fig:fig5_appendix_a}, for test cases 7--12.}
