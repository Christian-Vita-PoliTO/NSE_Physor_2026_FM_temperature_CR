...
\begin{abstract}
The fission matrix (FM) method enables whole-core reactor transport calculations within seconds by interpolating a precalculated database of Monte Carlo (MC)--tallied coefficients across temperature, instead of running a direct MC calculation for every configuration of interest. Interpolation implicitly assumes each coefficient depends only on the local temperature of the two cells it couples, an assumption that breaks down under large, spatially sharp temperature gradients; a Core Temperature Correction Ratio (CTCR) was previously introduced to compensate for this and shown to reduce the point-estimate discrepancy against direct reference calculations. A point estimate alone, however, cannot say whether a remaining discrepancy is a real, uncorrected bias or simply the statistical noise expected from the underlying Monte Carlo calculations. Answering that requires an uncertainty budget with two components: the reference case's own directly-reported Monte Carlo statistics, and the uncertainty of the \emph{interpolated} matrix itself --- a new object, assembled from the database's own finite-statistics matrices, that carries no Monte Carlo replicas of its own and therefore no uncertainty to read off any single output file. We construct the latter by resampling the database many times and recomputing the interpolated (and corrected) result at each repetition, comparing four ways of doing so: uncorrelated resampling of each coefficient's own reported variance; a zero-replica analytical model built from the branching structure of the fission process, under the simplifying assumption that only same-source-column coefficients are correlated; correlated resampling using a correlation matrix measured once, from a single Monte Carlo run, and reused across all database temperatures; and a genuine brute-force reconstruction of the whole database from 150 independent replicas. Validated against the brute-force result across 12 test cases, the analytical model reproduces the true $k_{\text{eff}}$ uncertainty to within a mean absolute deviation of 4.6\%, matching or exceeding the two more expensive alternatives, while requiring no Monte Carlo replicas beyond the nominal database already needed for interpolation. Combining both uncertainty sources, the correction ratio reduces the mean absolute $k_{\text{eff}}$ discrepancy by 10\% and the mean fission-source RMS discrepancy by 20\% at the point-estimate level across the 12 cases, and about half of the case-by-case discrepancies that remain after correction are statistically consistent with the combined noise floor.
\end{abstract}
...
This correction is calibrated from two additional, directly simulated FMs in which a single central assembly $C$ is set to the maximum (minimum) database temperature while the rest of the core is at the minimum (maximum) database temperature, shown in Figure~\ref{fig:dirac_distributions}. 
